#pragma warning disable MA0051, CA2201, S112 // Method is too long, System.Exception is not sufficiently specific
...
public static class WaitForDependenciesExtensions
{
        ...
        IAsyncDisposable
    {
            ...
            _ = Task.Run(async () =>
                {
                    ...
                    await foreach (ResourceEvent resourceEvent in resourceNotificationService.WatchAsync(stoppingToken))
                    {
                        if (!waitingResources.TryGetValue(resourceEvent.Resource, out ConcurrentDictionary<WaitOnAnnotation, TaskCompletionSource>? pendingAnnotations))
                        {
                            continue;
                        }

                        foreach ((WaitOnAnnotation waitOn, TaskCompletionSource tcs) in pendingAnnotations)
                        {
                            if (waitOn.States is { } states && states.Contains(resourceEvent.Snapshot.State?.Text, StringComparer.Ordinal))
                            {
                                pendingAnnotations.TryRemove(waitOn, out _);

                                _ = DoTheHealthCheckAsync(resourceEvent, tcs);
                            }
        ...
        private static async Task DoTheHealthCheckAsync(ResourceEvent resourceEvent, TaskCompletionSource tcs)
        {
            ...
            if (healthCheckAnnotation?.HealthCheckFactory is { } factory)
            {
                ...
                try
                {
                    ...
                    if (check is not null)
                    {
                        ...
                        {
                            HealthCheckResult result = await check.CheckHealthAsync(context, cancellationToken);
            ...
            try
            {
                ...
                {
                    ResiliencePipeline pipeline = CreateResiliencyPipeline();
